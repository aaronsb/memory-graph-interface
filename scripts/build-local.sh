...
TEMP_DIR="/tmp/memory-graph-interface"
mkdir -p "$TEMP_DIR"
...
if [ "$VERBOSE" = true ]; then
    if docker build -t memory-graph-interface:local .; then
    ...
    if docker build -t memory-graph-interface:local . > "$DOCKER_LOG" 2>&1; then
...
echo -e "\n${GREEN}Build complete!${NC} Image tagged as memory-graph-interface:local"
